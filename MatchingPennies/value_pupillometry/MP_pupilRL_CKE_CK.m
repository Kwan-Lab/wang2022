function MP_pupilRL_CKE_CK(dataIndex)
...
for ii = 1:nFiles
    ...
    if length(fn_pup) == 1
        ...
        %saveMLRmatpath = fullfile(savematpath,[fn_beh.name(1:end-7),'regRL_RPE_CK.mat']);
         saveMLRmatpath_change = fullfile(savematpath,[fn_beh.name(1:end-7),'regRL_CKE_change_CK.mat']);  % regression for pupil change
        savepupilfigpath = fullfile(dataIndex.BehPath{ii},[date(1:6),'_figs-pupil']);
        if ~exist(savepupilfigpath,'dir')
            mkdir(savepupilfigpath);
        end
        ...
        params.trigEvent= [NaN; choice(1:end-1,1)]; % c(n-1)
        ...
        params.trigEvent2= [NaN;reward(1:end-1)];   % r(n-1)
        
        % dQ
        params.trigEvent3 = stats_new.ql-stats_new.qr; % delta q
        %RPE
        params.trigEvent4= stats_new.rpe;
        % dK
        params.trigEvent5 = stats_new.ckl-stats_new.ckr;
        % CKE
        % 1-choice choice
        params.trigEvent6 = NaN(size(trials.go));
        params.trigEvent6(stats_new.c==-1) = 1-stats_new.ckl(stats_new.c==-1); % choosing left
        params.trigEvent6(stats_new.c==1) = 1-stats_new.ckr(stats_new.c==1);
        
        params.trigEvent7 = NaN(size(trials.go));
        for kk = 1:length(trials.left)
            if kk <= 20
                params.trigEvent7(kk) = sum(trials.reward(1:kk))/kk;
            else
                params.trigEvent7(kk) = sum(trials.reward(kk-19:kk))/20;
            end
        end
        
        params.trigEvent8=NaN(size(trials.left));
        for kk = 1:length(trials.left)
            params.trigEvent8(kk) = sum(trials.reward(1:kk));
        end
        params.trigEvent8 = params.trigEvent8/sum(trials.reward);
...
%         saveas(gcf, 'MLR-RL_RPE_fitall', 'fig');
        ...
        % pupil change
        reg_cr_CKE_change = linear_regr_PR(pupil.resp, pupil.respT, RL_event, params.trigTime, trialMask, params, trialData.cueTimes );
%% RPE controls    
%          params.ifplot = 0;
%          tlabel={'C(n)','QL-QR','C(n-1)','R(n-1)', 'RPE', 'QLC-QRC', 'RPE_C', 'Average reward', 'Cumulative reward'};
...
% left trials
        ...
        % C(n)
        %paramsPos.trigEvent = params.trigEvent(leftIndex);
        % C(n-1)
        paramsPos.trigEvent1= params.trigEvent(leftIndex);
        %R(n-1)
        paramsPos.trigEvent2= params.trigEvent2(leftIndex);
        %dQ
        paramsPos.trigEvent3 = params.trigEvent3(leftIndex); % delta q
        %RPE
        paramsPos.trigEvent4= params.trigEvent4(leftIndex);
        %dK
        paramsPos.trigEvent5 = params.trigEvent5(leftIndex);
        % CKE
        paramsPos.trigEvent6 = params.trigEvent6(leftIndex);
        %average reward
        paramsPos.trigEvent7 = params.trigEvent7(leftIndex);
        %cumulative reward
        paramsPos.trigEvent8=params.trigEvent8(leftIndex);
          % make matrix
        RL_event_pos = concat_event(paramsPos);
        
        paramsPos.trigTime = trialData.cueTimes(leftIndex);
        ...
        tlabel={'C(n-1)','R(n-1)','QL-QR', 'RPE', 'Average reward', 'Cumulative reward'};
...
%         saveas(gcf, 'MLR-RL_posRPE_fitall', 'fig');
        ...
        reg_cr_CKEleft_change = linear_regr_PR(pupil.resp, pupil.respT, RL_event_pos, paramsPos.trigTime, trialMask, paramsPos, trialData.cueTimes );
...
%          reg_cr_RPEpos_change_ctrl = linear_regr_ctrl(pupil.resp, pupil.respT, RL_event_pos,  params.trigTime, trialMask, params, tlabel );
        ...
        %% negative RPE trials
       % C(n)
        %paramsNeg.trigEvent = params.trigEvent(rightIndex);
        % C(n-1)
        paramsNeg.trigEvent1= params.trigEvent(rightIndex);
        %R(n-1)
        paramsNeg.trigEvent2= params.trigEvent2(rightIndex);
        %dQ
        paramsNeg.trigEvent3 = params.trigEvent3(rightIndex); % delta q
        %RPE
        paramsNeg.trigEvent4= params.trigEvent4(rightIndex);
        %dK
        paramsNeg.trigEvent5 = params.trigEvent5(rightIndex);
        % CKE
        paramsNeg.trigEvent6 = params.trigEvent6(rightIndex);
        %average reward
        paramsNeg.trigEvent7 = params.trigEvent7(rightIndex);
        %cumulative reward
        paramsNeg.trigEvent8=params.trigEvent8(rightIndex);
        
         % make matrix
        RL_event_neg = concat_event(paramsNeg);
        paramsNeg.trigTime = trialData.cueTimes(rightIndex);
...
%         saveas(gcf, 'MLR-RL_RPE_negfitall', 'fig');
        ...
        reg_cr_CKEright_change = linear_regr_PR(pupil.resp, pupil.respT, RL_event_neg, paramsNeg.trigTime, trialMask, paramsNeg, trialData.cueTimes );
...
%             'reg_cr_neg','reg_cr_RPEneg_ctrl','reg_cr_chosenQ','reg_cr_chosenQ_ctrl');
        ...
        save(saveMLRmatpath_change, 'reg_cr_CKE_change','reg_cr_CKEleft_change','reg_cr_CKEright_change');
