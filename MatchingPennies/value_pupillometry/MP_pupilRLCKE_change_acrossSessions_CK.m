function MP_pupilRLCKE_change_acrossSessions_CK(dataIndex, savefigpath);
...
for ii = 1:nFiles
    ...
    saveRegName = fullfile(savematpath,[fn_beh.name(1:end-7),'regRL_CKE_change_CK.mat']);
    
    if exist(saveRegName)
        load(saveRegName);
        
        reg_all.regr_time = reg_cr_CKE_change.regr_time;
        reg_all.numPredictor = reg_cr_CKE_change.numPredictor;
        reg_all.interaction = reg_cr_CKE_change.interaction;
        reg_all.nback = reg_cr_CKE_change.nback;
        all_coeff= cat(3,all_coeff, reg_cr_CKE_change.coeff);
        all_pval = cat(3,all_pval, reg_cr_CKE_change.pval);
...
tlabel1={'c(n-1)','r(n-1)','Q_L-Q_R', 'RPE', 'C_L-C_R', 'CPE', 'Average reward', 'Cumulative reward'};
