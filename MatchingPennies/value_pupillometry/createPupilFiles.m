...
% also plot a control figure to show the recording is aligned to behavior
% files
...
for ii = 1:nFiles
    pupCreated = 0;
    % load behavior files, use the behavior files before cut to include all
    % trials for alignment
    ...
    % check if only one file is found
        ...
        if length(pupilCSV) == 1
            ...
            fn = dir(fullfile(fn_beh.folder,[pupilMat.name(1:end-9),'_pup.mat']));
            if size(fn,1)>0
                pupCreated = 1;
            end
            ...
            if pupCreated == 0
                ...
                % check how many data points are determined as outlier
...
%                 title('Raw pupil size and center around scenario start');
