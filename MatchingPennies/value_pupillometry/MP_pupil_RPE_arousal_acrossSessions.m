function MP_pupil_RPE_arousal_acrossSessions(dataIndex, savefigpath)
...
for ii = 1:nFiles
    ...
    saveRegName = fullfile(savematpath,[fn_beh.name(1:end-7),'regRL_RPE_arousal.mat']);
...
tlabel={'c(n)','c(n-1)','r(n-1)','dQ', 'RPE', 'dK', 'CKE', 'Average reward', 'Cumulative reward'};
pvalThresh=0.01;
MP_plot_regrcoef_pupil_two(reg_cr_low,reg_cr_high,{'low','high'},pvalThresh,tlabel,xtitle);
print(gcf,'-dpng','MLR-RPE_arousal_averageSession');    %png format
saveas(gcf, 'MLR-RPE_averageSession', 'fig');
saveas(gcf, 'MLR-RPE_arousal_averageSession','svg');
...
print(gcf,'-dpng','MLR-RPE_sigSession');    %png format
saveas(gcf, 'MLR-RPE_sigSession', 'fig');
saveas(gcf, 'MLR-RPE_sigSession','svg');
