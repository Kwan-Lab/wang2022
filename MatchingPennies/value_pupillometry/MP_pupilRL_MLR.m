function MP_pupilRL_MLR(dataIndex)
...
for ii = 1:nFiles
    ...
    fn_latent = fullfile(dataIndex.BehPath{ii}, [dataIndex.LogFileName{ii}(1:end-4),'_FQRPElatentV.mat']);
    load(fn_latent);
    % load pupil files
...
%         saveas(gcf, 'MLR-RL', 'fig');
        ...
        % check
            ...
            close all;
        %% fit the model with different ITI
...
%         fieldname={'go'};
%         trialMask = getMask(trials,fieldname);
%         reg_cr_outcome=linear_regr( pupil.dia, pupil.t, [params.trigEvent params.trigEvent2 params.trigEvent3 params.trigEvent4 params.trigEvent5 params.trigEvent6 params.trigEvent7], params.trigTime, trialMask, params );
%         save(saveMLRmatpath_outcome, 'reg_cr_outcome');
%         tlabel={'C(n)','R(n)','C(n)xR(n)','QL-QR', 'QL+QR', 'C(n-1)','R(n-1)'};
%         params.xtitle = {'Time from outcome (s)'};
%        
%         MP_plot_regrRL_pupil(reg_cr_outcome,params.pvalThresh,tlabel,params.xtitle);
%         
%         print(gcf,'-dpng','MLR-RL-lag0_cut_fitall');    %png format
%         saveas(gcf, 'MLR-RL-lag0_cut_fitall', 'fig');
