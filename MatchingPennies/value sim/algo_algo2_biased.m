function stats=algo_algo2_biased(stats,params,x)
% % algo2 %
%PURPOSE:   Simulate computer opponent, algorithm 2, biased MP game
%AUTHORS:   AC Kwan 180423, Hongli Wang, 191204
%
%INPUT ARGUMENTS
...
if numel(c) > params.trial_back  %if there is enough choice history
    ...
    pval = myBinomTest(nom,denom,2/3);
    pl = nom/denom;
    ...
    for n = 1:params.trial_back
        ...
        pval = [pval myBinomTest(nom,denom,2/3)];
        pl = [pl nom/denom];
    ...
    %% player will choose based on conditional probability is furthest away from 2/3
    if ~isempty(pl)
        [~,max_idx]=max(abs(pl - 0.67));
        stats.pl(stats.currTrial,x) = 1-pl(max_idx);  %algorithm will strive for a non-match
    else
        stats.pl(stats.currTrial,x) = 1/3;
    end
