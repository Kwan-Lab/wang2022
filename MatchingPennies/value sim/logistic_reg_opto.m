function [output, negloglike]=logistic_reg_opto(stats,x,step_back) 
% % logistic_reg_opto %
...
YR=zeros(length(c),1);   % rewarded/no photostim control trials
YR=1*((c==1) & (r==1)) + (-1)*((c==-1) & (r==1));

NR=zeros(length(c),1);   % unrewarded/no photostim control trials
NR=1*((c==1) & (r==0)) + (-1)*((c==-1) & (r==0));

% generate regressor matrix
rmat=zeros(length(NR)-step_back,4*step_back);
for i=1+step_back:length(NR)       %trial..
    for j=1:step_back              %predictor..
...
output.b_reward_ctrl=b_ctrl(2:step_back+1);
output.b_unreward_ctrl=b_ctrl(step_back+2:2*step_back+1);

output.b_bias_opto=b_opto(1);
output.b_reward_opto=b_opto(2:step_back+1);
output.b_unreward_opto=b_opto(step_back+2:2*step_back+1);
...
for k=2:numel(c)
    ...
    if opto(k)==0
        ...
        b_temp = b_temp + nansum(output.b_reward_ctrl(1:backIdx).*YR(k-1:-1:k-backIdx));    %reward/right terms
        b_temp = b_temp + nansum(output.b_unreward_ctrl(1:backIdx).*NR(k-1:-1:k-backIdx));  %unreward/right terms
    elseif opto(k)==1
        b_temp = output.b_bias_opto;    %bias term
        b_temp = b_temp + nansum(output.b_reward_opto(1:backIdx).*YR(k-1:-1:k-backIdx));    %reward/right terms
        b_temp = b_temp + nansum(output.b_unreward_opto(1:backIdx).*NR(k-1:-1:k-backIdx));  %unreward/right terms
    end
