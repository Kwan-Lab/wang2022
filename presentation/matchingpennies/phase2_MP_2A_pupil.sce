...
preset int waterAmount_left = 78;
preset int waterAmount_right = 72;

preset int max_consecMiss = 200;                                                                                 ; #triggers end session, for mice, set to 20
int consecMiss = 0;
...
int rewardIndex = parameter_window.add_parameter("reward rate");
int meanNLIndex = parameter_window.add_parameter("mean NLs");
...
string path="C:\\Users\\KWANLAB\\Desktop\\Presentation\\hONGLI\\matchingpennies\\binomialtest_cheatsheet.txt";
...
loop
	int i = 1
until	consecMiss >= max_consecMiss || leftlick > 500 || rightlick > 500 || i > 1000
begin
	pupTrigger.present();
	port.send_code(portcode_pup, 5);
	...
	IRFlash.present();
	port.send_code(portcode_IR, 800);
  ...
  #for algorithm 1, need to do 5 binomial test first.
	...
	if response_manager.response_count()>0 then
		...
		IRFlash.present();
		port.send_code(portcode_IR, 100);
   ...
   loop until nLicks == 0 || numNolick>=maxNolick
	...
	parameter_window.set_parameter(rewardIndex,string(rewardRate));	
...
End.present();
port.send_code(portcode_end, 100);
