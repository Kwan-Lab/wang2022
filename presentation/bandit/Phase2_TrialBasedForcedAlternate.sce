##  Trial Based Alternation  Version 7/27/18
#
#NOTES:  Wait 1 sec, then go left or right - 100% Reward, 100% Switch
# HA, 12/1/18
#-------HEADER PARAMETERS-------#
scenario = "Phase2_Alternation";
...
#--------PCL---------------
begin_pcl;
display_window.draw_text("Phase 2- Alternation");
term.print("Starting time:");
term.print(date_time());

# PCL subroutines
string scenario =  "phase2_Alternation";
...
## Wait Cue Period  - Wait for no lick for at least 1 sec
	...
	begin
     ...
     begin
		...
		if (response_manager.last_response()==1) then nLicks = 0;
		else
...
## Window updates - details of block
	...
	if left_r+right_r>2 &&  nTrials_hiP_total>2 then
	 parameter_window.set_parameter(hiP_rateIndex,string(nTrials_hiP_total*100/(n_trial-indMiss))+"%");
