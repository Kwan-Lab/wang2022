function stats=algo_WSLS(stats,params)
% % algo_WSLS %
...
if stats.currTrial == 1   %if this is the first trial
    stats.pl(1) = 0.5;  %choose randomly
else
    if p >= rand(1)  %follow WSLS
        if stats.r(stats.currTrial-1)>0      %if last trial was rewarded
            stats.pl(stats.currTrial)=(stats.c(stats.currTrial-1)==-1);    %stay: prob(L)=1 if last trial was left, 0 otherwise
        elseif stats.r(stats.currTrial-1)==0  %if last trial was unrewarded
            stats.pl(stats.currTrial)=1-(stats.c(stats.currTrial-1)==-1);  %switch
