function funlog = optimviz_bandit(fun,x0,noise,MaxFunEvals,stats,filename)
% modified version of OPTIMIVIZ Optimizer visualization - fits with stats
% values instead of a linear vector. 
...
nSpace =30;
table.xVec = linspace(table.LB(1),table.UB(1),nSpace);
table.yVec = linspace(table.LB(2),table.UB(2),nSpace);
X= combvec(table.xVec,table.yVec)';
fprintf ('\nCalculating for %2.f different options\n',nSpace*nSpace)
for k=1:size(X,1)
    temp(k,1) = table.fun(X(k,:),stats);
end
table.zMat = reshape(temp,[numel(table.xVec),numel(table.yVec)])';
table.xx = [];
table.yy = [];
table.ymin = [];
table.ymin_true = table.fun([0.3 5],stats);
...
for n = MaxFunEvals
    for iOpt = 1:numel(funlog)  
        ...
        X = funlog{iOpt}.X(1:MaxFunEvals,:);
        
        % Re-evaluate Y with true function (assumed vectorized)
        for k=1:MaxFunEvals
            Y(k) = table.fun(X(k,:),stats);
        end
        
        table.h1 = hg(iOpt);
        table.h2 = hg2(iOpt);
        table.name = optims{iOpt}{1};
        optimplot(funlog{iOpt},table,n,stats,X(1:n,:), Y(1:n));
    end
