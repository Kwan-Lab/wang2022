function bandit_fittingDrift(dataIndex,save_path)
% %MP_fittingPerAnimal %
...
% 2. robust estimation methods (less sensitive to outliers)
%     L estimators, M estimators, S estimator and MM estimator,least absolute deviations (LAD), least median of
%     squares (LMS), least trimmed squares (LTS),
% 3. tonic pupil acitivity - learning rate/inverse temprature
...
for k=2:numel(model)
    ...
    if (runFit)
        for j = 1:numel(animalList)
            ...
            % get rid of the NaNs
            allIndex = 1:length(stats_all.c);
            NIndex = allIndex(isnan(stats_all.c));valueIndex = allIndex(~isnan(stats_all.c));
            stats_fit.c = stats_all.c(valueIndex,1);
            stats_fit.r =stats_all.r(valueIndex);
            window  =60; % fit the parameter every 10 trials
