function stats=algo_bayesian_twoparams(stats,params)
% % algo_bayesian_twoparams %
...
%           1) p_trans = probability of switch before criterion is met
%           2) p_crit = probability of switch after criterion is met
%
%OUTPUT ARGUMENTS
...
if stats.currTrial == 1  %if this is the first trial
    ...
    if correctChoice < criterion
        p = p_trans;  % criterion not met, transition expected at specified low rate
    else
        p = p_crit;    % criterion met, transition expected according to exponential distribution
    end
    
    % while criterion is not met, state transition probability is as specified by parameter
