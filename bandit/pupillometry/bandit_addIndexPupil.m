function newDataIndex = bandit_addIndexPupil(dataIndex)
% % addIndexLesion %
...
for b = 1:nFile
    ...
    date = dataIndex.LogFileName{b}(end-13:end-4);
    CSVfile = fullfile(dataIndex.LogFilePath{b},['*',date,'*.csv']);
    corresCSV = dir(CSVfile);
    if ~isempty(corresCSV)
        pupilIndex.pupil(b)=1;
        pupilIndex.pupilSide(b) = 2;        % pupil recording always on the right
    else
