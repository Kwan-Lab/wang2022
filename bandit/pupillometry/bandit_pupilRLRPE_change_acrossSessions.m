function bandit_pupilRLRPE_change_acrossSessions(dataIndex, savefigpath);
...
for ii = 1:nFiles
    ...
    saveRegName = fullfile(savematpath,[fn_beh.name(1:end-7),'regRL_RPE_change.mat']);
    if exist(saveRegName)
        ...
        % load control
        if ~exist('all_coeff_ctrl') && exist('reg_cr_RPE_change_ctrl')
        ...
        for uu = 1:length(fieldsName)
            all_coeff_ctrl.(fieldsName{uu}) = cat(3, all_coeff_ctrl.(fieldsName{uu}), reg_cr_RPE_change_ctrl.(fieldsName{uu}).coeff);
            all_pval_ctrl.(fieldsName{uu}) = cat(3, all_pval_ctrl.(fieldsName{uu}), reg_cr_RPE_change_ctrl.(fieldsName{uu}).pval);
        end
        end
...
print(gcf,'-dpng','MLR-RL_averageSession_RPE_change');    %png format
saveas(gcf, 'MLR-RL_averageSession_RPE_change', 'fig');


% plot updatedQ

MP_plot_regrcoef_pupil(reg_all_updatedQ,pvalThresh,tlabel2,xtitle);
print(gcf,'-dpng','MLR-RL_averageSession_updatedQ_change');    %png format
saveas(gcf, 'MLR-RL_averageSession_updatedQ_change', 'fig');
...
print(gcf,'-dpng','MLR-RL_RPE_change');    %png format
saveas(gcf, 'MLR-RL_RPE_change', 'fig');
...
print(gcf,'-dpng','MLR-RL_updatedQ_change');    %png format
saveas(gcf, 'MLR-RL_updatedQ_change', 'fig');
