function bandit_pupilRLRPE_acrossSessions_CK(dataIndex, savefigpath);
...
for ii = 1:nFiles
    ...
    if exist(saveRegName)
        ...
        % load control
        if ~exist('all_coeff_RPEctrl') && exist('reg_cr_RPE_ctrl')
            % initialize the control matrix
            fieldsName = fieldnames(reg_cr_RPE_ctrl);
            for tt = 1:length(fieldsName)
                all_coeff_RPEctrl.(fieldsName{tt}) = [];
                all_pval_RPEctrl.(fieldsName{tt}) = [];
            end
        ...
        for uu = 1:length(fieldsName)
            all_coeff_RPEctrl.(fieldsName{uu}) = cat(3, all_coeff_RPEctrl.(fieldsName{uu}), reg_cr_RPE_ctrl.(fieldsName{uu}).coeff);
            all_pval_RPEctrl.(fieldsName{uu}) = cat(3, all_pval_RPEctrl.(fieldsName{uu}), reg_cr_RPE_ctrl.(fieldsName{uu}).pval);
        end
        end
%         
...
%         end
        ...
        all_coeff_updatedQ = cat(3,all_coeff_updatedQ, reg_cr_chosenQ.coeff);
...
% print(gcf,'-dpng','MLR-RL_averageSession_posRPE');    %png format
% saveas(gcf, 'MLR-RL_averageSession_posRPE', 'fig');
% 
% MP_plot_regrcoef_pupil(reg_all_neg,pvalThresh,tlabel4,xtitle);
% print(gcf,'-dpng','MLR-RL_averageSession_negRPE');    %png format
% saveas(gcf, 'MLR-RL_averageSession_negRPE', 'fig');
...
% print(gcf,'-dpng','MLR-RL_averageSession_allRPE');    %png format
% saveas(gcf, 'MLR-RL_averageSession_allRPE', 'fig');
...
if exist('all_coeff_RPEctrl')
    reg_pval_ctrl = getBootstrp(all_pval_RPEctrl, 0.01);
...
print(gcf,'-dpng','MLR-RL_RPE_CK');    %png format
saveas(gcf, 'MLR-RL_RPE_CK', 'fig');
saveas(gcf, 'MLR-RL_RPE_CK','svg');
...
print(gcf,'-dpng','MLR-RL_updatedQ_CK');    %png format
saveas(gcf, 'MLR-RL_updatedQ_CK', 'fig');
saveas(gcf, 'MLR-RL_updatedQ_CK','svg');
...
print(gcf,'-dpng','MLR-RL_posRPE');    %png format
saveas(gcf, 'MLR-RL_posRPE', 'fig');
...
print(gcf,'-dpng','MLR-RLnegRPE');    %png format
saveas(gcf, 'MLR-RL_negRPE', 'fig');

% plot pos/neg together
reg_all.coeff = [all_coeff_pos(:,1,:),all_coeff_pos(:,5,:),all_coeff_neg(:,5,:)];
reg_all.pvalThresh = 0.01;
reg_all.nback = 0;
reg_all.numPredictor = 2;
reg_all.pval = [reg_sig_pos.pval(:,1,:),reg_sig_pos.pval(:,5,:),reg_sig_neg.pval(:,5,:)];
MP_plot_regr(reg_all,[],reg_all.pvalThresh,tlabel,xtitle);
print(gcf,'-dpng','MLR-RL_posnegRPE');    %png format
saveas(gcf, 'MLR-RL_posnegRPE', 'fig');
