function bandit_pupilMLR_acrossSessions(dataIndex, savefigpath)
...
% load the control
        if ~exist('all_coeff_future_ctrl') && exist('reg_cr_future_ctrl')
            ...
            fieldsName_future = fieldnames(reg_cr_future_ctrl);
        ...
        if exist('reg_cr_future_ctrl')
            ...
            for uu = 1:length(fieldsName_future)
                all_coeff_future_ctrl.(fieldsName_future{uu}) = cat(3, all_coeff_future_ctrl.(fieldsName_future{uu}), reg_cr_future_ctrl.(fieldsName_future{uu}).coeff);
                all_pval_future_ctrl.(fieldsName_future{uu}) = cat(3, all_pval_future_ctrl.(fieldsName_future{uu}), reg_cr_future_ctrl.(fieldsName_future{uu}).pval);
            end
...
% preprocessing the control coefficient and pval: bootstrap the pval to
% determine a baseline percennificant session
