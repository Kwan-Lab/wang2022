function bandit_pupilMLRleft(dataIndex)
...
for ii = 1:nFiles
    ...
    pup_name = fullfile(dataIndex.BehPath{ii}, ['*',date(1:6),'*_pupleft.mat']);
    fn_pup = dir(pup_name);
    if length(fn_pup) == 1
        
        savepupilfigpath = fullfile(dataIndex.BehPath{ii},[date(1:6),'_figs-pupilleft']);
        ...
        cd(savepupilfigpath);
        saveRegName_change = fullfile(savematpath,[fn_beh.name(1:end-7),'regCR_changeleft.mat']);  % regression for pupil change
  ...
  % interaction term
        ...
        params_future.trigEvent8 = [NaN;NaN;reward(1:end-2)]; % R(n-2)
            ...
            %reg_cr_futurectrl = linear_regr_ctrl(pupil.dia, pupil.t, future_event, params.trigTime, trialMask, params, tlabel);
        ...
        %save(saveRegName, 'reg_cr', 'reg_cr1','reg_cr2','reg_cr3','reg_cr_future', 'reg_cr_ctrl','reg_cr_future_ctrl');
