function check_pupilTriggerTimes_twopupil (stackInfo, trialData, savefigpath,side)
...
if side == 1
    title('Value should not deviate from diagonal (left)');
    print(gcf,'-dpng',[savefigpath,'\check-triggertiming_left']);    %png format
    saveas(gcf, [savefigpath,'/check-triggertiming_left'], 'fig');
elseif side == 2
    title('Value should not deviate from diagonal (right)');
    print(gcf,'-dpng',[savefigpath,'\check-triggertiming_right']);    %png format
    saveas(gcf, [savefigpath,'/check-triggertiming_right'], 'fig');   
end
